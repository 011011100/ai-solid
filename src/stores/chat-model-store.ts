...
import {getAllChatModel} from "../api/ollama-api.js";
...
function createChatModelStore() {
    ...
    function getAllModelName() {
        setModelNameList([]);
        // let {data} = useEventSource<OllamaChatModelTagsResponse>(getAllChatModel())
        let {data} = useEventSource<DeepseekChatModelTagsResponse>(getAllChatModel())
        createRoot(() => {
            createEffect(() => {
                let d = data()?.data;
                if (d) {
